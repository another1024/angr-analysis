...
class Project(object):
    ...
    def __init__(self, thing,
                 ...
                 **kwargs):
        '''
        angr=project()时候的调用，整个程序最开始的入口点
        '''
        ...
        if isinstance(thing, cle.Loader):
            ...
            '''
            进行一系列初始化检测后调用cle.loader进行加载程序，文件在cle下的loader.py中
            '''
        ...
        '''
        设置engines（默认使用的是simvex，这是simengines的一个子类，进行具体的模拟执行，在engines文件夹下的hub.py文件中，vex在engines文件夹中）
        下面是对engines对的初始化与插件加载
        '''
        ...
        '''
        进行程序analyses模块的初始化，主要进行程序cfg分析与程序恢复（在analyses的analysis.py中）
        下面是对analyses对的初始化与插件加载
        '''
        ...
        '''
        这是最早版本的simmamage现在已经不在主程序中使用，不过对于analyses还是需要他
        '''
        ...
        '''
        注册simprocedures类，这个类用于让用户书写子类进行自定义控制,同时初始化用户用钩子
        '''
