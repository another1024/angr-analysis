...
class ForwardAnalysis(object):
    ...
    def _analyze(self):
        ...
        self._pre_analysis()
        '''
        进行初始分析，创建第一个jobs，切出start块
        CFGBase类的函数，由CFGfast实现
        '''
    ...
    def _analysis_core_baremetal(self):
        ...
        if not self._job_info_queue:#判断job info队列是否为空
        ...
        while not self.should_abort:
            ...
            #获取job类信息job_info.job为job类
            try:
                self._pre_job_handling(job_info.job) #对job进行预处理 在CFGFast中实现的
